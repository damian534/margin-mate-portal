...
export function getFullTaxCalculation(annualIncome: number): TaxCalculation {
  const marginalRate = calculateMarginalTaxRate(annualIncome);
  const taxPayable = calculateTaxPayable(annualIncome);
  const effectiveRate = annualIncome > 0 ? (taxPayable / annualIncome) * 100 : 0;
  return { marginalRate, taxPayable, effectiveRate };
}
