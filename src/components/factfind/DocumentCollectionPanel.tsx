...
export function DocumentCollectionPanel({ leadId, isPreviewMode, primaryApplicantName, primaryApplicantEmail, primaryApplicantPhone, actionsSlot, coApplicantContact, onCoApplicantAdded, onCoApplicantRemoved }: DocumentCollectionPanelProps) {
  ...
  const fetchTemplates = async (brokerId?: string | null) => {
    if (isPreviewMode) return;
    // Scope templates to the lead's broker so super-admin viewers don't see
    // duplicate checklists pulled from other tenants.
    let q = (supabase as any)
      .from('document_templates')
      .select('id, name, items, broker_id')
      .order('display_order');
      ...
      if (!isPreviewMode) {
        ...
        if (accessToken) {
          ...
          await Promise.all(Object.entries(docsByApplicant).map(async ([applicantId, docs]) => {
            ...
            // Use the actual saved applicant id to scope the portal link to this person's docs only.
